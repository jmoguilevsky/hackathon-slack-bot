...
export function flowToBlocks(flow: FlowProject, connections: ConnectionListResponse): unknown {
  return [
...
const stepDescriptionByName: Record<string, (...t: any) => string> = {
  ...
  createSpreadsheetRow: ({ spreadsheetId, worksheetName }) =>
    `A new row is created in spreasheet ${spreadsheetId} in worksheet ${worksheetName}`,
...
export function getFlowBlocks(flow: FlowProject, connections: ConnectionListResponse): unknown {
